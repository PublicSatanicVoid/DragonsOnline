 ...
 public class QuestStep {
   ...
   public static QuestStep fromDocument(Document step, Quest quest) {
     return new QuestStep(step.getString("stepName"), 
         ...
         (List<QuestAction>)step.getList("actions", Document.class)
         .stream()
   ...
   public Document toDocument() {
     return (new Document("stepName", this.stepName))
       .append("trigger", this.trigger.toDocument())
       .append("actions", this.actions.stream().map(a -> a.toDocument()).collect(Collectors.toList()));
   }
   ...
   public void addBranchPoint(QuestTrigger trigger, QuestAction action) {
     ...
     int stepIndex = this.quest.getStepIndex(this);
     List<Document> steps = this.quest.getStepsAsDoc();
     ((Document)((Document)steps.get(stepIndex)).get("trigger", Document.class))
