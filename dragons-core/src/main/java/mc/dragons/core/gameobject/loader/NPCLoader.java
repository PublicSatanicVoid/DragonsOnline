...
import mc.dragons.core.util.StringUtil;
 ...
 public class NPCLoader extends GameObjectLoader<NPC> {
   ...
   public NPC loadObject(StorageAccess storageAccess) {
     ...
     NPC npc = new NPC(e, npcType.isPersistent() ? this.storageManager : (StorageManager)this.localStorageManager, npcType.isPersistent() ? storageAccess : (StorageAccess)this.localStorageManager.downgrade(storageAccess));
   ...
   public NPC registerNew(Entity entity, NPCClass npcClass) {
     return registerNew(entity, npcClass.getClassName(), npcClass.getName(), npcClass.getMaxHealth(), npcClass.getLevel(), 
         npcClass.getNPCType(), npcClass.hasAI(), npcClass.isImmortal());
   }
   ...
   public NPC registerNew(Entity entity, String className, String name, double maxHealth, int level, NPC.NPCType npcType, boolean ai, boolean immortal) {
     ...
     Document data = (new Document("_id", UUID.randomUUID()))
       ...
       .append("ai", Boolean.valueOf(ai))
       .append("immortal", Boolean.valueOf(immortal))
     ...
     StorageAccess storageAccess = npcType.isPersistent() ? this.storageManager.getNewStorageAccess(GameObjectType.NPC, data) : this.localStorageManager.getNewStorageAccess(GameObjectType.NPC, data);
   ...
   public void loadAllPermanent(boolean force) {
     ...
     this.storageManager.getAllStorageAccess(GameObjectType.NPC, new Document("$or", 
           Arrays.<NPC.NPCType>asList(NPC.NPCType.values())
           .stream()
           .filter(type -> (type.isPersistent() && type.isLoadedImmediately()))
