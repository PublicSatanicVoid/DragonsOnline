...
public class StorageUtil {
	...
	public static Document vecToDoc(Vector vec) {
		return (new Document()).append("x", Double.valueOf(vec.getX())).append("y", Double.valueOf(vec.getY())).append("z", Double.valueOf(vec.getZ()));
	}

	public static Document locToDoc(Location loc) {
		Document doc = vecToDoc(loc.toVector());
		return doc.append("world", loc.getWorld().getName()).append("pitch", Double.valueOf(loc.getPitch())).append("yaw", Double.valueOf(loc.getYaw()));
	}
