   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
     if (sender instanceof Player) {
       ...
       for (Map.Entry<Quest, QuestStep> entry : (Iterable<Map.Entry<Quest, QuestStep>>)user.getQuestProgress().entrySet())
         sender.sendMessage(ChatColor.GRAY + "- " + ChatColor.YELLOW + ((Quest)entry.getKey()).getQuestName() + 
             ChatColor.GRAY + " (Step: " + ((QuestStep)entry.getValue()).getStepName().trim() + ChatColor.GRAY + ")"); 
     } 
     return true;
   }
 }
