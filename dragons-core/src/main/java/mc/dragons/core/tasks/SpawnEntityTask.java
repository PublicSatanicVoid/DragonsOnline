   ...
   public void run() {
     ...
     for (User user : UserLoader.allUsers()) {
       if (user.getPlayer() == null || 
         user.getPlayer().getGameMode() == GameMode.CREATIVE || user.getPlayer().getGameMode() == GameMode.SPECTATOR)
         continue; 
       ...
       Map<String, Double> optimizedSpawnRates = (Map<String, Double>)spawnRates.entrySet()
         .stream()
       ...
       if (cap != -1) {
         ...
         entityCount = user.getPlayer().getNearbyEntities(searchRadius, searchRadius, searchRadius).stream()
           .map(e -> NPCLoader.fromBukkit(e))
           .filter(n -> (n != null))
           .filter(n -> (n.getNPCType() == NPC.NPCType.HOSTILE))
           .count();
       } 
       ...
       if (radiusCap != -1) {
         ...
         entityRadiusCount = user.getPlayer().getNearbyEntities(searchRadius, searchRadius, searchRadius).stream()
           .map(e -> NPCLoader.fromBukkit(e))
           .filter(n -> (n != null))
           .filter(n -> (n.getNPCType() == NPC.NPCType.HOSTILE))
           .count();
       ...
       for (Map.Entry<String, Double> spawnRate : optimizedSpawnRates.entrySet()) {
         ...
         priority++;
         if ((entityCount > cap && cap != -1) || (
           entityRadiusCount > radiusCap && radiusCap != -1))
           break; 
       } 
     } 
