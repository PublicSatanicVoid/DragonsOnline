...
public class PerformanceCommands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(label.equalsIgnoreCase("worldperformance")) {
			sender.sendMessage(ChatColor.DARK_GREEN + "World performance statistics for " + Dragons.getInstance().getServerName()
		...
		if(label.equalsIgnoreCase("serverperformance")) {
			new BukkitRunnable() { // some OperatingSystemMXBean operations lag the thread, so run asynchronously
				@Override public void run() {
					sender.sendMessage(ChatColor.DARK_GREEN + "Server performance statistics for " + Dragons.getInstance().getServerName()
							+ " @ " + new SimpleDateFormat("MM/dd/yyyy HH:mm").format(Date.from(Instant.now())));
					sender.sendMessage(ChatColor.GREEN + "Bukkit Version: " + ChatColor.GRAY + Bukkit.getVersion());
					sender.sendMessage(ChatColor.GREEN + "Uptime: " + ChatColor.GRAY + StringUtil.parseSecondsToTimespan(Dragons.getInstance().getUptime() / 1000));
		...
		if(label.equalsIgnoreCase("getprocessid")) {
			sender.sendMessage(ChatColor.GREEN + "Process ID: " + ChatColor.GRAY + ProcessHandle.current().pid());
		}
		
		if(label.equalsIgnoreCase("requestgc")) {
			long before = System.currentTimeMillis();
			System.gc();
			sender.sendMessage(ChatColor.GREEN + "Ran garbage collector in " + (System.currentTimeMillis() - before) + "ms");
		}
		
		if(label.equalsIgnoreCase("generatedump")) {
			CommandSender console = Bukkit.getConsoleSender();
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("==== BEGIN FULL SERVER DATA DUMP ====");
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("=== BEGIN USER DUMP ===");
			for(User user : UserLoader.allUsers()) {
				if(user.getPlayer() == null) {
					Dragons.getInstance().getLogger().info("USER " + user.getName() + " - " + user + " - OFFLINE");
				}
				else {
				Dragons.getInstance().getLogger().info("USER " + user.getName() + " - " + user + " - World " + user.getPlayer().getWorld()
						+ ", Loc " + StringUtil.locToString(user.getPlayer().getLocation()) + ", Access Level " + user.getActivePermissionLevel());
				}
			}
			Dragons.getInstance().getLogger().info("=== END USER DUMP ===");
			Dragons.getInstance().getLogger().info("");
			...
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("==== END FULL SERVER DATA DUMP ====");
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("");
			Dragons.getInstance().getLogger().info("");
